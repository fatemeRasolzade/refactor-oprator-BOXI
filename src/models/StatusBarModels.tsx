...
export const StatusBarElements: Record<string, ElementsValue> = {
  ...
  AllocationToDriver: {
    icon: <BsTruck size={30} className="text-[#0EA5E9]" />,
    text: "تخصیص به راننده",
  },
  Collected: {
    icon: (
      <div className="relative">
        <CgCheck size={25} className="absolute -top-5 -right-2 text-[#10B981]" />
        <BsBox size={30} className="text-[#10B981]" />
      </div>
    ),
    text: "جمع آوری شده",
  },
  FaildedCollected: {
    icon: (
      <div className="relative">
        <CgClose size={16} className="absolute -top-[.9rem] -right-2 text-red" />
        <BsBox size={30} className="text-red" />
      </div>
    ),
    text: "عدم جمع آوری موفق",
  },
  Uncollectible: {
    icon: <SlSocialDropbox size={30} className="text-yellow-500" />,
    text: "غیر قابل جمع آوری ",
  },
  Canceled: {
    icon: <TbBoxOff size={30} className="text-[#9CA3AF]" />,
    text: "لغو شده",
  },
  Undeliverable: {
