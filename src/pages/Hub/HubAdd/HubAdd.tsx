...
console.log(values)
   ...
   {(formik)=>(
     ...
     <div className='w-11/12 grid grid-cols-5 gap-2'>
       ...
       <div ><InputText title='کدهاب' name="code" handelChange={formik.handleChange} values={formik.values.code} important type={"text"}/>
       <ErrorMessage name='codeHub' render={(messege)=>(<span className='text-tomato'>{messege}</span>)}/>
       </div>
       <div ><InputText title='نام هاب' name="name" handelChange={formik.handleChange} values={formik.values.name} important type={"text"}/>
       <ErrorMessage name='nameHub' render={(messege)=>(<span className='text-tomato'>{messege}</span>)}/>
       </div>
        <div> <InputSelect text='نوع هاب' name="selectHubType" handelChange={formik.setFieldValue} values={formik.values.selectHubType} options={typeHub}/> 
       
        <ErrorMessage name='typeHub' render={(messege)=>(<span className='text-tomato  block mt-5'>{messege}</span>)}/>
        </div>
       <div >
        <InputSelect text='گونه هاب' name="selectHubCategory" handelChange={formik.setFieldValue} values={formik.values.selectHubCategory} options={catHub}/>
      
       </div>
        <div >
      
          <InputSelect text='هاب والد' name="selectParentHub" handelChange={formik.setFieldValue} values={formik.values.selectParentHub} options={selectHub}/> 
       ...
       <div><Checkbox title='فعال' name="isActive" handelChange={formik.handleChange} values={formik.values.isActive}/>
      
       </div>
       <div><Checkbox title='امکان تحویل مرسوله دارد' name="dropOffAllowed" handelChange={formik.handleChange} values={formik.values.dropOffAllowed}/>
      
       </div>
      <div >
         <InputSelect text='استان' name="selectState" handelChange={formik.setFieldValue} values={formik.values.selectState} options={selectProvince}/> 
     
      </div>
       <div >
        <InputSelect text='شهر' name="selectCity" handelChange={formik.setFieldValue} values={formik.values.selectCity} options={citys}/> 
      
       </div>
       <div >
       <InputSelect text='منطقه' name="selectRegion" handelChange={formik.setFieldValue} values={formik.values.selectRegion} options={provinceLoc}/> 
      
       </div> 
       <div ><InputText title='پلاک' name="plateNumber" handelChange={formik.handleChange} values={formik.values.plateNumber} type={"number"}/>
       
       </div>
       <div ><InputText title='آدرس 1' name="addressLine1" handelChange={formik.handleChange} values={formik.values.addressLine1} type={"text"}/>
      
       </div>
       <div className='!col-span-2 ' ><InputText title='آدرس 2' name="addressLine2" handelChange={formik.handleChange} values={formik.values.addressLine2} type={"text"}/>
