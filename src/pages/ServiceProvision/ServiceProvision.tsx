...
const ServiceProvision = () => {
  ...
  const data =
    serviceList?.content &&
    serviceList.content.map((item: any) => {
      return {
        id: item?.id ? item?.id : "",
        ...
        validDateFrom: item?.validDateFrom.year ? (
          <span>{`${item?.validDateFrom.year}/${item?.validDateFrom.month}/${item?.validDateFrom.day}`}</span>
        ) : (
          ""
        ),
